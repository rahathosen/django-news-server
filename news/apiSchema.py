...
class Query(graphene.ObjectType):
    ...
    def resolve_post_by_category(self, info, category_id, **kwargs):
        return Post.objects.filter(category_id=category_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_sub_category(self, info, sub_category_id, **kwargs):
        return Post.objects.filter(sub_category_id=sub_category_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_country(self, info, country_id, **kwargs):
        return Post.objects.filter(country_id=country_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_division(self, info, division_id, **kwargs):
        return Post.objects.filter(division_id=division_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_district(self, info, district_id, **kwargs):
        return Post.objects.filter(district_id=district_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_cityCorporation(self, info, cityCorporation_id, **kwargs):
        return Post.objects.filter(cityCorporation_id=cityCorporation_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_upozila(self, info, upozila_id, **kwargs):
        return Post.objects.filter(upozila_id=upozila_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_zip_postal_code(self, info, zip_postal_code_id, **kwargs):
        return Post.objects.filter(zip_postal_code_id=zip_postal_code_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_tag(self, info, tag_id, **kwargs):
        return Post.objects.filter(tag_id=tag_id).filter(status=1, editor_reviewed=1)
    
    def resolve_post_by_reporter(self, info, author_id, **kwargs):
        return Post.objects.filter(author_id=author_id).filter(status=1, editor_reviewed=1)
    
    
    def resolve_last_8_post_by_category(self, info, category_id, **kwargs):
        return Post.objects.filter(category_id=category_id).filter(status=1, editor_reviewed=1)[:8]
    
    def resolve_last_8_post_by_sub_category(self, info, sub_category_id, **kwargs):
        return Post.objects.filter(sub_category_id=sub_category_id).filter(status=1, editor_reviewed=1)[:8]
    ...
    def resolve_top_8_post_by_category_this_week(self, info, category_id, **kwargs):
        return Post.objects.filter(category_id=category_id, 
                                   created_at__gte=datetime.date.today()-datetime.timedelta(days=7)).filter(status=1, editor_reviewed=1)
    ...
    def resolve_filter_post(self, info, category_id, sub_category_id, country_id, division_id, district_id, cityCorporation_id, upozila_id, zip_postal_code_id, tag_id, author_id, **kwargs):
        return Post.objects.filter(category_id=category_id, sub_category_id=sub_category_id, 
                                   country_id=country_id, division_id=division_id, district_id=district_id, 
                                   cityCorporation_id=cityCorporation_id, upozila_id=upozila_id, 
                                   zip_postal_code_id=zip_postal_code_id, tag_id=tag_id, 
                                   author_id=author_id).filter(status=1, editor_reviewed=1)
